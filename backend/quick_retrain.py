...
def load_dataset(paralytic_path, normal_path, max_images=300):
    ...
    if os.path.exists(paralytic_path):
        ...
        for i, image_file in enumerate(image_files[:max_images]):
            if i % 50 == 0:
                print(f"Processing paralytic image {i+1}/{min(max_images, len(image_files))}")
    ...
    if os.path.exists(lfw_path):
        ...
        for person_dir in person_dirs:
            ...
            for image_file in image_files[:2]:  # Max 2 per person
                if processed >= max_images:
                    break
                    
                if processed % 50 == 0:
                    print(f"Processing normal image {processed+1}/{max_images}")
    ...
    return paralytic_features, paralytic_labels, normal_features, normal_labels

def train_model(paralytic_features, paralytic_labels, normal_features, normal_labels):
    """Train the model"""
...
def main():
    print("🚀 Starting Quick Facial Paralysis Model Retraining...")
    print("=" * 60)
    ...
    # Load datasets
    paralytic_features, paralytic_labels, normal_features, normal_labels = load_dataset(
        paralytic_path, normal_path, max_images=200
    )
    ...
    # Train model
    model = train_model(paralytic_features, paralytic_labels, normal_features, normal_labels)
